...
def diff_sys_lockdown (f, t, beta, gamma, t_hammer, t_dance):
    """Funktion zur Umsetzung des Differentialgleichungssystem des SIR-Modells.
    ...
    dI = beta_lockdown(t, beta, t_hammer, t_dance) * S * I - gamma * I
    dR = gamma*I
    return dS, dI, dR
...
def main():
    ...
    F = odeint(diff_sys, f_0, t, args=(beta, gamma))
    #Abspeichern der drei Lösungsfunktionen
    S, I, R = F.T     #Verwendung von .T zur transponierung der Lösungsmatrix
    ...
    F_lock = odeint(diff_sys_lockdown, f_0, t, args=(beta, gamma, t_hammer, 
                    t_dance))
    S_lock, I_lock, R_lock = F_lock.T #analog wie zuvor
    ...
    for i in range(iter_variaton):
        ...
        F_var = odeint(diff_sys, f_0, t_long, args=(beta, gamma))
        S_var, I_var, R_var = F_var.T
...
#a)
#Die Vergrößerung von Beta führt zu einer Vergrößerung des Maximums der
#Infizierten Personen. Außeredem wird bei größeren Beta das Maximum schneller 
#bzw. zu einem früheren Zeitpunkt erreicht.
#Eine Veringerung von Beta hingegen führt zu einer Verzögerng des Erreichen des 
#Maximums und dem Verringeren der Höhe des Maximums.

#b)
#Graphisch abgelesener Wert für R_0: 1.702
#entspricht bei gamma=0.1 einem beta=0.1702

#c)
#Der simulierte Lockdown führt ab t_hammer zu einer Reduzierung des Anstiegs der
#Infizierten und gleichzeitig zu einer Reduzierung des Abfallens der 
#infizierbaren Personen. Im betrachteten Beispiel (expliziter Ablauf Lockdown 
#und gewählte Parameter) kommt es zu einer Verzögerung des Erreichen des 
#Maximums der infizierten Personen und einer Abflachung des Maximums. Der 
#Maximalwert der Infizierten wird dabei auf unter die Hälfte reduziert.

#Bei einer kompletten Aufhebung der Maßnahmen und dem daraus folgenden Ansteigen
#der Infektionswahrscheinlichkeit kommt es nur zu einer Verzögerung des 
#Erreichen des Maximums. Der Maximalwert der Infizierten Personen ist identisch 
#zum Verlauf der Pandemie ohne Lockdown.
